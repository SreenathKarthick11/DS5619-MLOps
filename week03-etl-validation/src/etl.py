...
def report_gen(vio_objs):
    violation_counts = {
        "expect_column_not_null": 0,
        "expect_column_positive": 0,
        "expect_column_in_set": 0,
        "expect_column_unique": 0,
    }
    ...
    for violation in vio_objs:
        violation_counts[violation.expectation] += 1
        violation_rows[violation.expectation].append(violation.row_index)

    expectations = []

    for expectation in violation_counts:
        expectations.append({
            "expectation": expectation,
            "n_violations": violation_counts[expectation],
            "row_indices": violation_rows[expectation],
        })

    return {
        "n_violations": len(vio_objs),
